 ...
 */
export function renderConnectivity(answer) {
    if (answer.status !== "supported" || answer.intent !== "connectivity") {
        throw new Error(`renderConnectivity requires status "supported" and intent "connectivity", ` +
            `but received status "${answer.status}" and intent "${answer.intent}".`);
    }
    const sections = [];
    // Resumen
    sections.push(`<section class="answer-connectivity__summary">` +
        `<h3>Resumen</h3>` +
    ...
    if (answer.stages.length > 0) {
        const stageItems = answer.stages
            .map((stage) => `<li class="route-stage">` +
            `<dl>` +
            ...
            `</li>`)
            .join("");
        sections.push(`<section class="answer-connectivity__stages">` +
            `<h3>Etapas</h3>` +
            `<ol>${stageItems}</ol>` +
            `</section>`);
    }
    ...
    if (answer.warnings.length > 0) {
        const warningItems = answer.warnings
            ...
            .join("");
        sections.push(`<section class="answer-connectivity__warnings">` +
            `<h3>Advertencias</h3>` +
            `<ul>${warningItems}</ul>` +
            `</section>`);
    }
    ...
    if (answer.sources.length > 0) {
        const sourceItems = answer.sources
            ...
            `</li>`)
            .join("");
        sections.push(`<section class="answer-connectivity__sources">` +
            `<h3>Fuentes</h3>` +
            `<ul>${sourceItems}</ul>` +
            `</section>`);
    }
    ...
    if (answer.verifiedAt) {
        sections.push(`<section class="answer-connectivity__verified-at">` +
            `<p><strong>Verificado:</strong> ${escapeHtml(answer.verifiedAt)}</p>` +
            `</section>`);
    }
    return (`<article class="answer-connectivity">` +
        sections.join("") +
        `</article>`);
}
