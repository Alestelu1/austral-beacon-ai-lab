 ...
 */
export function renderDestination(answer) {
    if (answer.status !== "supported") {
        throw new Error(`renderDestination requires status "supported", ` +
    ...
    // Resumen
    sections.push(`<section class="answer-destination__summary">` +
        `<h4>Resumen</h4>` +
    ...
    if (geoContext) {
        sections.push(`<section class="answer-destination__geographic">` +
    ...
    if (culturalContext) {
        sections.push(`<section class="answer-destination__cultural">` +
    ...
    if (answer.warnings.length > 0) {
        const warningItems = answer.warnings
            ...
            .join("");
        sections.push(`<section class="answer-destination__warnings">` +
            `<h4>Advertencias</h4>` +
            `<ul>${warningItems}</ul>` +
            `</section>`);
    }
    ...
    if (answer.sources.length > 0) {
        const sourceItems = answer.sources
            ...
            `</li>`)
            .join("");
        sections.push(`<section class="answer-destination__sources">` +
            `<h4>Fuentes</h4>` +
            `<ul>${sourceItems}</ul>` +
            `</section>`);
    }
    ...
    if (answer.suggestedInternalLinks.length > 0) {
        const linkItems = answer.suggestedInternalLinks
            ...
            .join("");
        sections.push(`<section class="answer-destination__links">` +
            `<h4>Enlaces sugeridos</h4>` +
            `<ul>${linkItems}</ul>` +
            `</section>`);
    }
    // Confianza
    sections.push(`<section class="answer-destination__confidence">` +
    ...
    if (answer.verifiedAt) {
        sections.push(`<section class="answer-destination__verified-at">` +
            `<p><strong>Verificado:</strong> ${escapeHtml(answer.verifiedAt)}</p>` +
            `</section>`);
    }
    return (`<article class="answer-destination">` +
        sections.join("") +
        `</article>`);
}
